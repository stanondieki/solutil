  ...
  const fetchUsers = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Map backend user structure to frontend expectations
        const mappedUsers = (data.users || []).map((user: any) => {
          // Debug date fields
          console.log(`User ${user.name} date fields:`, {
          ...
          let joinDate = 'N/A'
          const dateField = user.createdAt || user.dateJoined || user.registrationDate
          ...
          if (dateField) {
            try {
              ...
              console.error('Date parsing error for user', user.name, ':', error)
            }
          }
          ...
          return {
            ...
            role: user.userType === 'client' ? 'customer' : user.userType || 'customer',
            status: user.isActive === false ? 'suspended' : (user.userType === 'provider' ? user.providerStatus || 'active' : 'active'),
            joinDate,
